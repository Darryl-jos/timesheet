...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $sel_date = $edit_data['start_date'];
    $sel_start_time = substr($edit_data['start_time'], 0, 5);
    $sel_end_time = substr($edit_data['end_time'], 0, 5);
    $sel_work_desc = $edit_data['work_description'];
    
    $s_dt = new DateTime($edit_data['start_date'] . ' ' . $edit_data['start_time']);
    $e_dt = new DateTime($edit_data['end_date'] . ' ' . $edit_data['end_time']);
    $diff_seconds = $e_dt->getTimestamp() - $s_dt->getTimestamp();
    $diff_hours_raw = $diff_seconds / 3600;
    
    $expected_hours = 0;
    if ($diff_hours_raw >= 24) $expected_hours = $diff_hours_raw + 3;
    elseif ($diff_hours_raw > 16) $expected_hours = $diff_hours_raw + 2;
    ...
    if (!empty($sel_date) && !empty($sel_start_time) && !empty($sel_end_time)) {
        ...
        $c_stmt = $conn->prepare("SELECT start_date, start_time, end_date, end_time FROM timesheets WHERE engineer_id = ? AND id != ? AND CONCAT(start_date, ' ', start_time) < ? AND CONCAT(end_date, ' ', end_time) > ? LIMIT 1");
        $c_stmt->bind_param("iiss", $current_user_id, $edit_id, $new_end_dt_str, $new_start_dt_str);
        $c_stmt->execute();
...
.topbar { position: sticky; top: 0; z-index: 500; background: #ffffff; padding: 15px 20px; display: flex; align-items: center; justify-content: space-between; border-radius: 8px; flex-wrap: wrap; gap: 10px; }
...
.time-input-wrap .dropdown-icon { position: absolute; right: 0; top: 0; width: 30px; height: 100%; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 11px; color: #666; z-index: 5; }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    let dbMealBreaks = "<?php echo htmlspecialchars($sel_meal_breaks); ?>";
    let actualBreaksInput = document.getElementById('actual_meal_breaks');
    let mealCheckbox = document.getElementById('meal_break_checkbox');
    let breaksSelect = document.getElementById('meal_breaks_select');

    if (parseInt(dbMealBreaks) > 0) {
        actualBreaksInput.value = dbMealBreaks;
        mealCheckbox.checked = true;
    }

    calculateMealBreaks();

    if (parseInt(dbMealBreaks) > 0 && currentMaxBreaks > 1) {
        breaksSelect.style.display = 'block';
        breaksSelect.value = dbMealBreaks;
    }
    ...
    mealCheckbox.addEventListener('change', function() {
        if (this.checked) {
            if (currentMaxBreaks > 1) {
                breaksSelect.style.display = 'block';
                actualBreaksInput.value = breaksSelect.value;
            } else {
                breaksSelect.style.display = 'none';
                actualBreaksInput.value = "1";
            }
        } else {
            breaksSelect.style.display = 'none';
            actualBreaksInput.value = "0";
        }
        updateDurationPreview();
    });
    breaksSelect.addEventListener('change', function() {
        actualBreaksInput.value = this.value;
        updateDurationPreview();
    });
